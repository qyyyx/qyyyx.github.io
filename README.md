...
| 数论与组合 | 费马小定理、关灯游戏、多格骨牌铺砖、骑士巡游问题、完全平方数、天平称重问题 |
| 概率统计 | 概率实验室（概率-集1） |
| 应用问题 | 经济问题 |
| 趣味与思维 | 工作记忆训练游戏、测测你的反应有多快！ |
...
2. 打开 `index.html`，在 `COURSEWARE` 数组里加一条：
   ...
   字段说明：
   - `cat`（必填）：`geo` 几何 / `alg` 代数 / `num` 数论 / `prob` 概率统计 / `app` 应用 / `fun` 趣味与思维。
   - `isNew`（可选）：`true` 时卡片右上角显示红色「NEW」角标。
3. 提交并推送，GitHub Pages 自动更新。

> **课件数量无需手动维护（回写规则）**：`COURSEWARE` 数组是课件数量的**唯一真相源**。页面顶部「共 N 个交互课件」与分享卡 `meta`（`description`/`og`/`twitter`）描述都由 `COURSEWARE.length` 自动派生（见 `index.html` 的 `syncMeta()`）；静态 `meta` 已写成**不带数字的永久措辞**作为不跑 JS 的爬虫兜底，运行时再回写精确数量。**新增 / 删除课件时只改 `COURSEWARE` 数组即可，不要再在任何地方硬编码课件数字**（历史上这里曾因手动数字漏改而长期停留在「17 个」）。
